...
function mapPick(row,pan=true){
 ...
 $('#map-place-info').textContent=row.program+' · '+row.region+(row.height!==undefined?` · ${row.height} moh. · ${row.points} poeng`:'')+` · ${row.lat.toFixed(5)}, ${row.lon.toFixed(5)}`;
 if(pan)parkMap.getView().animate({center:ol.proj.fromLonLat([row.lon,row.lat]),zoom:12,duration:350});
}
function mapResults(rows){
 $('#map-results').innerHTML=rows.length?rows.map((r,i)=>`<button type="button" class="secondary" data-map-result="${i}">${esc(r.ref)} · ${esc(r.name)}</button>`).join(''):'Ingen treff.';
...
async function loadMapPoints(){
 ...
 try{
  ...
  $('#map-data-status').textContent=`${data.items.length} av ${data.total} steder i utsnittet${data.truncated?' · Zoom inn for å vise alle':''} · Register hentet ${data.updated.slice(0,10)}.`;
...
$('#map-use-own').onclick=()=>{
 ...
 $('#portable-session').scrollIntoView({behavior:'smooth',block:'center'});toast('Referansen er valgt. Trykk Start aktivering når du er klar.');
};
...
$('#map-home').onclick=()=>{try{const p=gridPoint(state.settings.grid);if(!p)throw Error();parkMap.getView().animate({center:ol.proj.fromLonLat([p.lon,p.lat]),zoom:9,duration:350});}catch{toast('Legg inn lokator under Min stasjon.',true);}};
$('#map-update').onclick=action(async()=>{const b=$('#map-update');b.disabled=true;$('#map-data-status').textContent='Henter oppdaterte park- og toppregistre …';try{await api('portable-map/update',{});await loadMapPoints();}finally{b.disabled=false;}});
