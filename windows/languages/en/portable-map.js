...
function mapPick(row,pan=true){
 ...
 $('#map-place-info').textContent=row.program+' · '+row.region+(row.height!==undefined?` · ${row.height} m above sea level · ${row.points} points`:'')+` · ${row.lat.toFixed(5)}, ${row.lon.toFixed(5)}`;
 if(pan)parkMap.getView().animate({center:ol.proj.fromLonLat([row.lon,row.lat]),zoom:12,duration:350});
}
function mapResults(rows){
 $('#map-results').innerHTML=rows.length?rows.map((r,i)=>`<button type="button" class="secondary" data-map-result="${i}">${esc(r.ref)} · ${esc(r.name)}</button>`).join(''):"No matches.";
...
async function loadMapPoints(){
 ...
 try{
  ...
  $('#map-data-status').textContent=`${data.items.length} of ${data.total} locations in this view${data.truncated?" · Zoom in to show all":''} · Directory retrieved ${data.updated.slice(0,10)}.`;
...
$('#map-use-own').onclick=()=>{
 ...
 $('#portable-session').scrollIntoView({behavior:'smooth',block:'center'});toast("Reference selected. Press Start activation when ready.");
};
...
$('#map-home').onclick=()=>{try{const p=gridPoint(state.settings.grid);if(!p)throw Error();parkMap.getView().animate({center:ol.proj.fromLonLat([p.lon,p.lat]),zoom:9,duration:350});}catch{toast("Enter a grid locator under My station.",true);}};
$('#map-update').onclick=action(async()=>{const b=$('#map-update');b.disabled=true;$('#map-data-status').textContent="Retrieving updated park and summit directories …";try{await api('portable-map/update',{});await loadMapPoints();}finally{b.disabled=false;}});
