...
const listenerUI = (()=>{
  ...
  let current={running:false,processing:false,results:[],level_db:-90,peak_db:-90,message:"Select an audio source to get started.",model_ready:false};
  function render(data){
    ...
    $('#audio-runtime').textContent=[data.model_ready?"Local speech model ready":"Speech model missing",data.device?"Source: "+data.device:'',data.dropped?data.dropped+" audio segments skipped because recognition could not keep up":'',data.peak_db>-1?"Audio is clipping – lower the input level":''].filter(Boolean).join(' · ');
    ...
    if(key!==lastResults){
      ...
      $('#audio-candidates').innerHTML=suggestions.size?[...suggestions.values()].slice(0,12).map(c=>`<button class="audio-candidate" data-heard-call="${esc(c.call)}"><strong>${esc(c.call)}</strong><span>${c.kind==='fonetisk'?"Interpreted from spelling":"Found in the text"} · ${esc(stamp(c.time))}</span><small>${esc(c.evidence)}</small></button>`).join(''):empty(data.results.length?"No clear callsigns found":"Waiting for radio speech","Read the text below or try a clearer audio segment.");
      $('#audio-transcripts').innerHTML=data.results.length?data.results.map(r=>`<div class="audio-transcript"><div><span class="small muted">${esc(stamp(r.time))} · ${r.duration} s · ${esc(r.source)}</span><p>${esc(r.text)}</p></div><button class="mini-button" data-audio-play="${esc(r.id)}">▶ Play</button></div>`).join(''):empty("No speech recognized yet","Select an audio source and start listening, or try a short WAV recording.");
    }
    $('#audio-use-call').disabled=!moduleEnabled('log');
    $('#audio-use-call').textContent=moduleEnabled('log')?"Use in a new contact":"Select the Logbook module first";
  }
  async function refreshDevices(){
    ...
    $('#audio-device').innerHTML="<option value=\"\">Select audio source …</option>"+devices.map(d=>`<option value="${d.id}" data-name="${esc(d.name)}">${d.loopback?"PC audio":"Audio input"} · ${esc(d.name)}</option>`).join('');
    const previous=devices.find(d=>d.name===chosen);
    if(previous)$('#audio-device').value=String(previous.id);
    if(!devices.length)throw new Error("No available audio inputs found in Windows.");
    devicesLoaded=true;
  ...
  async function poll(){
    ...
    try{
      ...
      render(data);
    }catch(error){$('#audio-status').textContent="Lost contact with the listener";$('#audio-error').textContent=error.message;}
    finally{busy=false;}
  }
  ...
  $('#audio-start').onclick=action(async()=>{
    ...
    if(!selected)throw new Error("Select the audio source the program should listen to.");
  ...
  $('#audio-file').onchange=action(async event=>{
    ...
    try{
      if(file.size>12000000)throw new Error("Select a WAV file smaller than 12 MB and no longer than 60 seconds.");
  ...
  document.addEventListener('click',action(async event=>{
    ...
    if(play){
      ...
      if(current.running){await api('audio/stop',{});session=null;await poll();throw new Error("Listening has stopped before playback. Press Play again when the last audio segment is complete.");}
  ...
  $('#audio-use-form').onsubmit=action(event=>{
    ...
    if(!moduleEnabled('log'))throw new Error("Select Logbook under Select modules first.");
    const call=$('#audio-confirm-call').value.trim().toUpperCase();
    if(!/^[A-Z0-9/]{3,32}$/.test(call)||!/[A-Z]/.test(call)||!/[0-9]/.test(call))throw new Error("Check the callsign.");
    openQso(null,call);
  });
